...
function install_manager_system()
{
    ...
    # local iptables_conf=${ROOT_PATH}/etc/sysconfig/iptables       # disabled by diwen
...
{
    ## copy rc.update                               # abolished by diwen
...
function initrd_repack()
{
    ...
    log "Abolished the Navida Old Show Driver ..."
    local blk_list=${TMP_PATH}/initrd/etc/modprobe.d/blacklist.conf
    [ -f $blk_list ] && {
        log "add Navida Driver to blacklist ..."
        echo -e "# nouveau Navida driver\nblacklist nouveau" >> $blk_list
    }
    local etc_blk_list=${ROOT_PATH}/etc/modprobe.d/blacklist.conf
    [ -f $etc_blk_list ] && {
        log "modify currect system etc modprobe blacklist ..."
        grep -q "nouveau" $etc_blk_list
        [ "$?" != "0" ] && {
            echo -e "# nouveau Navida driver\nblacklist nouveau" >> $etc_blk_list
        }
    }
...
function parted_device()
{
    ...
    log "====================================================================\n" \
    "  LABEL    \t\t 1GB     \t 2 GB    \t 4 GB    \t 60 GB  \n" \
    "  /boot    \t\t 100 MB   100 MB  \t 100 MB    \t 500 MB  \n" \
    "  /        \t\t 550 MB    1.2 GB  \t 2 GB    \t 10 GB  \n" \
    "  system   \t 50 MB   50 MB  \t 200 MB    \t 3 GB  \n" \
    "  prog_bin \t 200 MB   300 MB  \t 1G    \t 15 GB  \n" \
    "  prog_etc \t 50 MB   \t 50 MB   \t 100 MB    \t 1GB  \n" \
    "  prog_log \t 50 MB   \t 100 MB  200 MB    \t 30 GB  \n" \
    "===================================================================="
    cf_size=$(string_to_number $cf_size)    # cf_size is bytes
    local start_part_offset=0

    echo "=============== Check Disk Volume ================="
    echo $cf_size
    echo "=============== Check Disk Volume ================="
    sleep 2
    ...
    if [ "$auto" != "yes" ];then
        ...
        if (($cf_size >= 59500000000))  # >= 50G, is 60 G ssd disk, 60000000000 bytes
        then
            cf_size=$((59500+$start_part_offset))
            boot_size=500
            root_size=10000
            system_size=3000
            prog_bin_size=15000
        ...
        then
            cf_size=$((4000+$start_part_offset))
            boot_size=100
            root_size=2000
            system_size=200
            prog_bin_size=1500
