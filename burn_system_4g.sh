...
Usage=$(echo "Usage: burn_system_4g.sh [Options...] do all things with none arguments \n"\
       "Options:\n"\
       "-a|--auto auto install\n" \
...
rpm -q glibc-common
[ "$?" != "0" ] && { echo "no glibc-common, try to reinstall it ..."; \
                        yum -y install glibc-common; }
echo "aleady installed glibc-common, we reinstall it ..."
yum -y reinstall glibc-common

if [ "$install_type" != "-s" ];then
    echo "run basesystem_update_4g.sh -c"
    sh ./update/basesystem_update_4g.sh -c
fi
...
# ===========================================================================
    # Here, disabled the old navidia display driver
...
if [ "$install_type" == "-s" ];then
    ...
    sh ./update/basesystem_update_4g.sh ${install_to_cf} ${install_type}
fi
...
if [ -n $SYSTEM_PATH ] && [ -n $ROOT_PATH ]; then
    ...
    [ ! -d "$SYSTEM_PATH/conf/rpmdb_bak" ] && {
        mkdir -p $SYSTEM_PATH/conf/rpmdb_bak
    }
