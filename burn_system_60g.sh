...
yum -y install bzip2-libs.i686 openssl.i686 glibc.i686 zlib.i686
...
#mysql config file                          # diabled by diwen
...
if [ -n $SYSTEM_PATH ] && [ -n $ROOT_PATH ]; then
    ...
    # echo -e "1234abcd!@#$@vmediax\n1234abcd!@#$@vmediax" | passwd root
    # echo -e "1234abcd\!\@\#\$\@vmediax\n1234abcd\!\@\#\$\@vmediax" | passwd root
    # '!' is '\0041'
...
# install img software pkg
[ -d ./burn_tmp ]&& rm -rf burn_tmp
mkdir -p burn_tmp
cd burn_tmp
FILE_LIST=$(find ../burn_data -type f -name "*.img")
for i in $FILE_LIST
do
    bzcat $i | cpio -idu
done
